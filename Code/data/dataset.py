...
class Vocab:
    ...
    def generate_embedding(self,data_path,vector_dim):

        embedding=nn.Embedding(self.size,vector_dim)
        ...
        for line in file:
            row = line.split(' ')
            if row[0] in self.vocab_dic:
                embedding.weight[self.vocab_dic[row[0]]].data = torch.tensor([float(k) for k in row[1:]])
                embedding.weight[self.vocab_dic[row[0]]].data.requires_grad_(False)

        return embedding
    @property
    def size(self):
        return len(self.vocab_dic)
